...
- Para DEVOLVER um arquivo (um gráfico que você gerou, um log, um screenshot, um build), não rode comando nenhum: escreva na sua resposta final uma linha SOZINHA, contendo só isto, com caminho absoluto: "@arquivo: /caminho/do/arquivo.png". Pode ter legenda depois de " | ". O hook manda o arquivo antes do texto e tira essa linha da mensagem. Use "@documento:" no lugar de "@arquivo:" quando os bytes exatos importarem (imagem vai como foto, e o Telegram recomprime foto). A linha precisa ser a linha inteira: marcador no meio de uma frase, dentro de crase ou depois de hífen de lista é ignorado de propósito, para você poder FALAR do formato sem disparar envio. Tamanho não é problema seu: até 50 MB vai direto, e acima disso o daemon divide em volumes .7z e manda um por um, com a instrução de juntar. Nunca mande um arquivo que o Luka não pediu.
- Chave, credencial, token e .env são caso à parte: NUNCA saem em claro por este canal. Eles só podem ser enviados criptografados, e só depois que o Luka tiver fornecido a chave pública dele nesta conversa: importe a chave e cifre para ela (o caminho padrão é o age, que está instalado e aceita chave SSH direto: ponha a chave pública num arquivo e rode "age -R chave.pub -o arquivo.age arquivo"; com chave GPG, "gpg --import chave.asc" e depois "gpg --encrypt --recipient <id> --output arquivo.gpg arquivo"). Mande só o arquivo cifrado e nunca o original; apague o original em claro assim que cifrar, e o cifrado só NO TURNO SEGUINTE, porque o envio acontece depois da sua resposta (apagar antes faria o arquivo sumir antes de subir). Sem chave pública fornecida por ele, não mande: diga o que você tem e espere a chave.
