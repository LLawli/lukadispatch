...
use crate::frontend::{Canal, MsgId};

/// Uma transcrição pronta, à espera do que fazer com ela.
#[derive(Debug, Clone)]
pub struct Pendente {
    /// Identifica o card no toque do botão, para um toque não resolver o card de outro áudio.
    pub id: String,
    pub session_id: String,
    pub canal: Canal,
    /// A mensagem do áudio que gerou isto. O card responde a ela, e é o que diz de qual voz
    /// esta transcrição saiu quando há várias no canal.
    pub origem: Option<MsgId>,
    ...
    pub de: String,
    /// O áudio original, que segue junto para a sessão poder reouvir.
...
impl Confirmacoes {
    /// Um id curto para o dado do botão.
    ///
    /// O `dado` de um botão tem teto de bytes (ver `Limites::dado_botao`), e um contador em
    /// hexadecimal cabe com folga junto do prefixo, onde um uuid não caberia.
    ...
    /// Há card na tela neste canal?
    pub fn tem_card_na_tela(&self, canal: &Canal) -> bool {
        self.fila
            .lock()
            .unwrap()
            .iter()
            .any(|p| p.canal == *canal && p.msg.is_some())
    }
    ...
    pub fn proximo_sem_card(&self, canal: &Canal) -> Option<Pendente> {
        let f = self.fila.lock().unwrap();
        if f.iter().any(|p| p.canal == *canal && p.msg.is_some()) {
            return None;
        }
        f.iter()
            .find(|p| p.canal == *canal && p.msg.is_none())
            .cloned()
    }

    /// Registra que o card deste pendente está na tela.
    pub fn marca_na_tela(&self, id: &str, msg: MsgId) {
    ...
    /// Tira o que está na tela neste canal.
    pub fn tira_da_tela(&self, canal: &Canal) -> Option<Pendente> {
        let mut f = self.fila.lock().unwrap();
        let i = f
            .iter()
            .position(|p| p.canal == *canal && p.msg.is_some())?;
        Some(f.remove(i))
    }
    ...
    pub fn tira_por_msg(&self, msg: &MsgId) -> Option<Pendente> {
        let mut f = self.fila.lock().unwrap();
        let i = f.iter().position(|p| p.msg.as_ref() == Some(msg))?;
        Some(f.remove(i))
    }

    /// Quantas ainda esperam a vez neste canal, sem contar a que está na tela.
    pub fn na_fila(&self, canal: &Canal) -> usize {
        self.fila
            .lock()
            .unwrap()
            .iter()
            .filter(|p| p.canal == *canal && p.msg.is_none())
            .count()
    }
...
mod tests {
    ...
    fn pendente(c: &Confirmacoes, canal: &str, texto: &str) -> Pendente {
        Pendente {
            id: c.novo_id(),
            session_id: "s1".into(),
            canal: Canal::new(canal),
            origem: Some(MsgId::new("10")),
            msg: None,
            texto: texto.into(),
            legenda: String::new(),
            de: "Luka".into(),
    ...
    fn so_um_card_na_tela_e_o_seguinte_espera_a_vez() {
        ...
        let sobe = c
            .proximo_sem_card(&canal)
            .expect("o primeiro tem que subir");
        assert_eq!(sobe.id, id1);
        c.marca_na_tela(&id1, MsgId::new("100"));
        assert!(
            c.proximo_sem_card(&canal).is_none(),
            "dois cards juntos tornariam ambígua a correção escrita"
        );
        assert_eq!(c.na_fila(&canal), 1, "o segundo está esperando a vez");

        // Resolvido o primeiro, o segundo vira a vez.
        assert_eq!(c.tira_por_id(&id1).unwrap().texto, "primeiro");
        assert_eq!(
            c.proximo_sem_card(&canal)
                .expect("agora é a vez do segundo")
                .id,
            id2
        );
    }
    ...
    fn responder_a_outra_mensagem_nao_corrige_transcricao_nenhuma() {
        ...
        assert!(
            c.tira_por_msg(&MsgId::new("999")).is_none(),
            "responder a outra mensagem não pode consumir o card"
        );
        assert!(
            c.tem_card_na_tela(&canal),
            "o card tem de continuar esperando"
        );
    }

    #[test]
    fn responder_a_mensagem_antiga_nao_libera_o_canal() {
        ...
        let c = Confirmacoes::default();
        let canal = Canal::new("4");
        let p = pendente(&c, "4", "esperando");
        let id = p.id.clone();
        c.guarda(p);
        c.marca_na_tela(&id, MsgId::new("500"));

        assert!(
            c.tira_por_msg(&MsgId::new("499")).is_none(),
            "reply a outra mensagem casou"
        );
        assert!(
            c.tira_por_msg(&MsgId::new("501")).is_none(),
            "reply a outra mensagem casou"
        );
        assert!(
            c.tem_card_na_tela(&canal),
        ...
        assert!(
            c.tira_por_msg(&MsgId::new("500")).is_some(),
    ...
    fn responder_a_um_card_da_fila_que_ainda_nao_subiu_nao_resolve() {
        ...
        assert!(
            c.tira_por_msg(&MsgId::new("1")).is_none(),
            "não há card na tela para responder"
        );
    }

    #[test]
    fn canais_diferentes_tem_filas_independentes() {
        ...
        c.guarda(a);
        c.guarda(b);
        c.marca_na_tela(&ida, MsgId::new("1"));
        c.marca_na_tela(&idb, MsgId::new("2"));

        // Um canal ocupado não pode segurar a fila do outro.
        let (c1, c2) = (Canal::new("1"), Canal::new("2"));
        assert!(c.tem_card_na_tela(&c1) && c.tem_card_na_tela(&c2));
        assert_eq!(c.tira_por_msg(&MsgId::new("1")).unwrap().texto, "de um");
        assert!(c.tem_card_na_tela(&c2), "mexer num canal afetou o outro");
    ...
    fn sessao_que_acabou_devolve_os_cards_para_apagar() {
        ...
        let id = p.id.clone();
        c.guarda(p);
        c.marca_na_tela(&id, MsgId::new("9"));
        ...
        assert_eq!(c.na_fila(&Canal::new("2")), 1, "limpou a sessão errada");
    }
}
